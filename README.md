...
本节以 `803bc8997fd6ff097a730b5c3658f71483ff09a5` 之前的版本为基线，记录 1.2.0 这一轮围绕悬浮球触发、识别弹窗交互和设置页整理做出的变化：
...
- **匹配类动作更直接**：连续数字、邮箱、链接只匹配并选择第一个结果；当前动作无匹配时会继续跳到下一项，直到找到可执行动作或一轮结束。
...
- **设置页按标签整理**：主页设置按入口、识别、搜索、调试分组，降低长页面混杂感。
...
- 识别弹窗下拉动作已改为可配置循环动作，支持全选、连续数字、邮箱、链接和取消选择
...
- 调整悬浮球锁定高度、单手优化与角度阈值
- 切换预制调试文本并预览 BigBang
- 配置悬浮球触发方式、下拉动作顺序、前后文追加按钮显示、搜索源、词典源、OCR 语言和 OCR 白名单
- 开启识别调试日志
- 选择图片进入 OCR 调试
...
- 进入 BigBang 后可下拉循环执行可配置动作，底栏可重进 OCR、关闭弹窗、可选显示前后文追加，右下角可选择目标语言并翻译当前正文
